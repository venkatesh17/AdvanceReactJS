...
class Billpayment extends React.Component {
    ...
    confirmDeletion =(callback, index)=> {
      swal({
          ...
          console.log("deletionConfirmed", deletionConfirmed)
          if(callback(index))
  ...
  confirmEdditing(callback, index, clearData){
   ...
   clearFormData(){
    ...
    render() {
      return <div className="container billpayment">
        <h1 style={{
            fontSize: "80px",
        ...
        {this.state.data[0]?
        <div>
         <p className="caption">Reported (14 april 2016)</p>
        <table>
            <thead>
              <tr>
                  <th>Project</th>
       ...
       { this.state.data.map((info, index) =>
         <tr key={index}>
                  ...
                  <td  
                    style= {{color:'black'}}>
                    <button onClick={()=>{this.confirmDeletion(this.deleteRecord, index)}}>Delete</button>
                  </td>
      </tr>       
       )
      }
